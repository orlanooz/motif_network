...
class GraphUtils(object):
    ...
    def construct_training_graph(self, filename=None):
        if filename is None:
            filename = os.path.join(self.model_path, "edges_train.dat")
        edge_list_u_v = []
        edge_list_v_u = []
    ...
    def neighbors(self, node):
        """
        Retrieves the neighbors of a node from the graph G.
    ...
    def get_random_walks_restart(self, datafile, hits_dict, percentage, maxT, minT):
        if datafile is None:
            datafile = os.path.join(self.model_path,"edges_train.dat")
