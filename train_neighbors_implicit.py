...
def initialize_embeddings(data, node_u, node_v, embedding_dim):
    ...
    for node in node_u:
        ...
        #Average site_fingerprints and add it to material_vectors
        fps_list = []
        if data[node[1:]]['motif_details'] == None: continue
        for motif_info in data[node[1:]]['motif_details'].values():
            fps = motif_info['site_fingerprint']
            fps_list.append(fps)
        num_dimensions = len(fps_list[0])
        sum_vector = [0] * num_dimensions
        for fps in fps_list:
            sum_vector = [sum_val + fps_val for sum_val, fps_val in zip(sum_vector, fps)]
        num_fps = len(fps_list)
        average_fps = [x / num_fps for x in sum_vector]
            
        at_vector = vectorize_composition(comp)
        combined_vector = np.concatenate((at_vector, average_fps))
...
def skip_gram(center, contexts, negs, node_list, lam, pa):
    ...
    for u in I_z.keys():
        ...
        X =np.dot(V, Theta.T).item()
        sigmod = stable_sigmoid(X)

        gradient = pa * lam * (I_z[u] - sigmod) * Theta
        update += gradient

        node_list[u]['context_vectors'] += gradient
        try:
...
#        data = pickle.load(f)
    with open("/home/anoj/work/network/data/perovskite_data/modified_perovs_data.pkl", 'rb') as f:
        data = pickle.load(f)
...
#        data = pickle.load(f)
    ...
    embeddings_dir = os.path.join(args.model_path, 'embeddings_imp')
    ...
    for iteration in range(args.max_iter):
        ...
        for u, v, w in edge_list:
            # Process `u`
            for index in range(len(context_dict_u[u])):
                    ...
                    for z in context_u:
                        ...
                        loss += tmp_loss
            # Process `v`
            for index in range(len(context_dict_v[v])):
